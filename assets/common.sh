...
get_current_version() {
  ...
  echo "$folder_contents" | jq --arg v "$regex" '[.children[].uri | capture($v)] | sort_by(.version | split(".") | map(tonumber)) | [.[length-1] | {version: .version}]'
}
...
get_all_versions() {
  ...
  echo "$folder_contents" | jq --arg v "$regex" '[.children[].uri | capture($v)] | sort_by(.version | split(".") | map(tonumber)) | [.[] | {version: .version}]'
}
...
get_files() {
  ...
  echo "$folder_contents" | jq --arg v "$regex" '[.children[].uri | capture($v)] | sort_by(.version  | split(".") | map(tonumber)) | [.[] | {uri: .uri, version: .version}]'
}
